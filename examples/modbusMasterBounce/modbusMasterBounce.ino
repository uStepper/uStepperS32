...
void setup() {
  ...
  modbus.writeSingleRegister(REG_RUN_CURRENT, 30);  // 30% current
  modbus.writeSingleRegister(REG_MODE, 3);          // Relative angle mode

  writeFloat(REG_MAX_ACCEL, 2000.0);    // Acceleration in fullsteps/s²
  writeFloat(REG_MAX_VELOCITY, 200.0);  // Velocity in fullsteps/s
}
